...
export default async function AdminOffersHeroPage({ params }: AdminOffersPageProps) {
  ...
  return (
    <AdminShell locale={locale}>
      ...
      <section style={sectionStyle}>
        <SectionTitle story="ADMIN-009" title="Manage offers data" />
        <div style={gridTwo}>
          {offers.map((offer) => (
            <article key={offer.id} style={{ ...surfaceCardStyle, display: "grid", gap: 10 }}>
              ...
              <div style={{ color: "var(--muted)" }}>
                {offer.price !== null && offer.price !== undefined ? `${offer.price.toFixed(2)}` : "No price set"}
              </div>
              {offer.description ? (
                <div style={{ fontSize: "0.9rem", color: "var(--muted)" }}>{offer.description.substring(0, 100)}...</div>
              ) : null}
              <FormModal
              ...
              >
                <form action={upsertOfferAction} style={{ display: "grid", gap: 14 }}>
                  ...
                  <textarea
                    ...
                    style={inputStyle}
                  />
                  <AvatarDropField name="avatar" label="Offer avatar" defaultValue={offer.avatar ?? ""} />
        ...
        <FormModal buttonLabel="Add new offer" title="Create offer" description="Create a new offer.">
          <form action={upsertOfferAction} style={{ display: "grid", gap: 14 }}>
            <input type="hidden" name="locale" value={locale} />
            <input name="title_en" placeholder="Offer title" style={inputStyle} required />
            <input name="price" type="number" min="0" step="0.01" placeholder="Price" style={inputStyle} />
            <textarea name="description_en" rows={4} placeholder="Description" style={inputStyle} />
